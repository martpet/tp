...
export function loginInPopupWindow(provider: IdentityProvider) {
  const loginUrl = `${apiUrl}${apiPaths.login}?provider=${provider}`;
  const windowName = 'loginWindow';
  const width = 600;
  const height = 650;
  ...
  return new Promise<void>((resolve) => {
    const loginWindow = window.open(loginUrl, windowName, features);

    const listener = ({ origin, data }: MessageEvent) => {
      if (origin !== apiUrl || data !== loginWindowSuccessData) {
        return;
      }
      resolve();
      loginWindow?.close();
